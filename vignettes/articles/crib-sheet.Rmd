...
Each entry says which lesson introduced it, so you can go back to the explanation whenever a one-line reminder is not enough.
...
| `mutate(linelist, sex = if_else(tolower(sex) == "female", "Female", "Male"))` | `if_else()` picks between two values per row; `tolower()` flattens case. The standard recoding pair. | M6 |
...
Every plot is three parts: the data, the `aes()` mapping, and a `geom_` that draws it.
...
| `start_report()` | Puts the report template in your working directory and opens it. | M14 |
...
| `use_local_library()` | Move your package library out of OneDrive, if installs keep failing. |
