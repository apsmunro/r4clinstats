...
This guide takes you from a blank computer to your first lesson. It
assumes no prior experience with R. Set aside about fifteen minutes the
first time.
...
- **R** is the engine. Download it from
  [CRAN](https://cran.r-project.org/) and install it like any other
  program.
- **RStudio** is the comfortable workspace you run R in. Download
  [RStudio Desktop](https://posit.co/download/rstudio-desktop/) and
  install that too.

Open RStudio once both are in place. You will see a panel called the
**Console**, with a `>` prompt. That is where the commands below go:
type or paste a line, then press Enter.
...
The two colons look stranger than they are: `package::function()` is R’s
way of saying “this function, out of this package”, and it saves loading
the package first. It is a command to type, not a web address, and there
is nothing to click or search for.
...
**“No tutorials found for package ‘r4clinstats’.”** Your R session is
holding an older view of the package. In RStudio, choose **Session ▸
Restart R**, then run
[`library(r4clinstats)`](https://github.com/apsmunro/r4clinstats) and
try `learn("m0")` again.

**Install fails on a work or managed computer.** Some workplaces store
your documents in OneDrive, or lock down where programs may write, which
R does not get along with. Run this once:
...
It sets up a private folder for your R packages in a location that
always works, then asks you to restart R. After restarting, repeat step
2.

**A package will not install from CRAN.** Behind some hospital and
